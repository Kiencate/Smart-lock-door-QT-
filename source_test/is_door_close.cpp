...
int main( int argc, char *argv[])
{
    ...
    while ( i < length ) {     
        ...
        try
        {
            ...
            if(std::stoi(status["is_face_detected"].dump()) == 1 || std::stoi(status["is_password_success"].dump()) == 1 || std::stoi(status["is_rfid_success"].dump()) == 1)
            {
        ...
        catch(nlohmann::json::parse_error& ex){ std::cerr << "is_door_close: open file: parse error at byte " << ex.byte << std::endl;} 
