...
void CheckStatus::run()
{
    ...
    is_timeout_touch=json_object_get_int(json_object_object_get(status_json_obj,"timeout_touch")) == 1? true:false;
    close(fd_status_json);
    emit JsonChangestatus(is_person, wifi_config, is_door_closed, face_detect, right_password, rfid_success,is_start_face_detect,is_wifi_connected, is_start_config_wifi,is_timeout_touch); 
    json_object_put(status_json_obj); 
    goto loop;
}
CheckStatus::~CheckStatus()
